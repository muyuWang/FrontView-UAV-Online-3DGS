#!/usr/bin/env python3
"""Run matched Mountains fallback-repair controls on GPUs 4-7."""
...
OUTPUT_ROOT = (
    runner.ROOT
    / "Logs_mountains_far_depth_goal_8_13/landmark_fallback_repair_620"
)
FIXED_FAR_MASKS = (
    runner.ROOT
...
METHODS = {
    "A_stage35": {},
    "B_repair_real": {
        "causal_landmark_memory": memory("fallback_repair"),
    },
    "C_repair_shuffled": {
        "causal_landmark_memory": memory("fallback_repair", shuffle=True),
    },
    "D_all_queries_real": {
...
def main():
    ...
    manifest = {
        ...
        "protocol": (
            "matched 620-frame mapping; fallback repair preserves every "
            "originally valid DepthCov row; real versus shuffled historical "
            "metric depths preserve UV, count, candidate locations, and budget"
        ),
        "tag": tag,
    ...
    if successful:
        ...
        summary["repair_causal_control"] = runner.metric_delta(
            results["B_repair_real"], results["C_repair_shuffled"]
        )
        summary["repair_versus_all_queries"] = runner.metric_delta(
            results["B_repair_real"], results["D_all_queries_real"]
        )
