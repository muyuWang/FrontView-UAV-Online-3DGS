#!/usr/bin/env python3
"""Run paired Mountains causal dual-responsibility experiments on GPUs 4-7."""
...
OUTPUT_ROOT = (
    runner.ROOT / "Logs_mountains_far_depth_goal_8_13/dual_responsibility_620"
)
FIXED_FAR_MASKS = (
    runner.ROOT
...
METHODS = {
    ...
    "C_dual_tracking_real": {
        "causal_dual_responsibility": DUAL,
        "causal_metric_birth": TRACKING,
    },
    "D_dual_tracking_shuffled": {
        "causal_dual_responsibility": DUAL,
        "causal_metric_birth": {**TRACKING, "shuffle_evidence": True},
    },
}

# Each row is serial on one GPU. Reversed real/shuffled order controls drift.
PAIR_SCHEDULE = {
    "4": ("A_stage35", "B_dual_no_tracking"),
    "5": ("D_dual_tracking_shuffled", "C_dual_tracking_real"),
    "6": ("C_dual_tracking_real", "D_dual_tracking_shuffled"),
    "7": ("A_stage35", "C_dual_tracking_real"),
}
...
def shared_process_env(gpu: str, cpu_threads: int = 1):
...
def main():
    ...
    manifest = {
        ...
        "protocol": (
            "same-GPU serial matched pairs; equal proposal budget; tracking "
            "replaces fallback proxies one-for-one; PSNR/SSIM only"
        ),
        "base_commit": "23b1731",
    ...
    with ThreadPoolExecutor(max_workers=len(jobs_by_gpu)) as executor:
        futures = {
            executor.submit(
                run_gpu_sequence, args, batch_root, gpu, jobs, tag
            ): gpu
        ...
        for future in as_completed(futures):
            ...
            print(
                f"GPU {gpu}: {len(results)} runs, {results[-1]['status']}", flush=True
            )
