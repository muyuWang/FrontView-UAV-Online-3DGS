#!/usr/bin/env python3
"""Run matched Mountains footprint-preserving depth reanchoring controls."""
...
OUTPUT_ROOT = runner.ROOT / "Logs_mountains_far_depth_goal_8_13/footprint_reanchor_620"
FIXED_FAR_MASKS = (
    runner.ROOT
...
def tracking(*, shuffle_binding=False):
    return {
        "enabled": True,
        "history_frames": 4,
        "max_features": 2048,
...
METHODS = {
    "A_stage35": {},
    "B_no_motion_floor": {
        "coverage_recovery": {"depth_fallback_motion_floor_enabled": False},
    },
    "C_reanchor_real": {
        "coverage_recovery": {"depth_fallback_motion_floor_enabled": False},
    ...
    "D_reanchor_shuffled": {
        "coverage_recovery": {"depth_fallback_motion_floor_enabled": False},
        "causal_metric_birth": tracking(shuffle_binding=True),
    },
}

# The real/shuffled pair is reversed to expose order drift on otherwise idle GPUs.
PAIR_SCHEDULE = {
    "4": ("A_stage35", "B_no_motion_floor"),
    "5": ("B_no_motion_floor", "C_reanchor_real"),
    "6": ("C_reanchor_real", "D_reanchor_shuffled"),
    "7": ("D_reanchor_shuffled", "C_reanchor_real"),
}
...
def main():
    ...
    manifest = {
        ...
        "protocol": (
            "same-GPU serial pairs; equal RGB proxy pixels, proposal budget, and "
            "Gaussian count opportunity; real versus depth-binding shuffle; no LPIPS"
        ),
        "base_commit": "23b1731",
